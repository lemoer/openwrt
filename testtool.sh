...
make V=s PATH=$PATH:$(pwd)/tmpenv/staging_dir/host/bin HOST_FIXUP=libtool HOST_BUILD_PREFIX=$(pwd)/$tmpbuild/host TOPDIR=$(pwd)/tmpenv -j 1 -C tools/$toolname/ compile
